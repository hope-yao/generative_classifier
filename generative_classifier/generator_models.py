...
def norb96_encoder(x, latent_dim, reuse=False):
    '''
    encoder for mnist digits
    :param x:
    :param latent_dim:
    ...
    with tf.variable_scope('Enc', reuse=reuse):
        output['net0'] = x
        output['net1'] = net1 = slim.conv2d(x, 32, 5, 2)
        output['net2'] = net2 = slim.conv2d(net1, 64, 5, 2)
        output['net3'] = net3 = slim.conv2d(net2, 128, 5, 2)
        output['net4'] = net4 = slim.conv2d(net3, 256, 5, 2)
        output['net5'] = net5 = slim.conv2d(net4, 512, 5, 2)

        output['end'] = net6 = slim.fully_connected(slim.flatten(net5), latent_dim, activation_fn=None)
    return output


def norb96_decoder(sample_sub, label, reuse=False):
    '''
    decoder with subnetworks for mnist digits
    :param sample_sub:
    :param label:
    ...
    for i, sample_i in enumerate(sample_sub):
        with tf.variable_scope('Dec{}'.format(i), reuse=reuse):
            h1 = slim.fully_connected(sample_i, 512 * 3 * 3, activation_fn=tf.nn.relu)
            h2 = slim.conv2d_transpose(tf.reshape(h1, (batch_size, 3, 3, 512)), 64, 5, 2, activation_fn=tf.nn.relu)
            h3 = slim.conv2d_transpose(h2, 256, 5, 2, activation_fn=tf.nn.relu)
            h4 = slim.conv2d_transpose(h3, 128, 5, 2, activation_fn=tf.nn.relu)
            h5 = slim.conv2d_transpose(h4, 64, 5, 2, activation_fn=tf.nn.relu)
            h6 = slim.conv2d_transpose(h5, 32, 5, 2, activation_fn=tf.nn.relu)
            out = slim.conv2d_transpose(h6, 1, 5, 1, activation_fn=None)
...
def norb48_encoder(x, latent_dim, reuse=False):
    '''
    encoder for mnist digits
    :param x:
    :param latent_dim:
...
def norb48_decoder(sample_sub, label, reuse=False):
    '''
    decoder with subnetworks for mnist digits
    :param sample_sub:
    :param label:
